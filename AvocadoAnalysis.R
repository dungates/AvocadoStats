list.files(path = "/Users/dunk/AvocadoStats")

df <- read.csv("/Users/dunk/AvocadoStats/avocado.csv")

original_avocado_df <- df
...
vol_type <- df %>% dplyr::group_by(type) %>% summarise(avg.vol=mean(Total.Volume))  %>% mutate(pct=prop.table(avg.vol) * 100) 
...
seasonal_df <- original_avocado_df

seasonal_df$month_year <- format(as.Date(original_avocado_df$Date), "%Y-%m")
seasonal_df$month <- format(as.Date(original_avocado_df$Date), "%m")
seasonal_df$year <- format(as.Date(original_avocado_df$Date), "%Y")
...
conv_patterns <- seasonal_df %>% select(monthabb, AveragePrice, type) %>% filter(type == "conventional") %>%
  dplyr::group_by(monthabb) %>% dplyr::summarize(avg=mean(AveragePrice)) %>%
...
conv_pat_yearly <- seasonal_df %>% select(year, monthabb, AveragePrice, type) %>% filter(type == "conventional", year == c("2015", "2016", "2017")) %>%
  dplyr::group_by(year, monthabb) %>% dplyr::summarize(avg=mean(AveragePrice)) %>%
...
std_conv <- seasonal_df %>% select(year, monthabb, AveragePrice, type) %>% filter(type == "conventional", year == c("2015", "2016", "2017")) %>%
  dplyr::group_by(year, monthabb) %>% dplyr::summarize(std=sd(AveragePrice)) %>%
...
r_avg <- seasonal_df %>% dplyr::group_by(year, monthabb) %>%  select(type, year, monthabb, AveragePrice) %>% 
  filter(type == "conventional", year == c("2015", "2016", "2017")) %>%
  dplyr::group_by(year, monthabb) %>%
  dplyr::summarize(avg=mean(AveragePrice))
...
r_avg_org <- seasonal_df %>% dplyr::group_by(year, monthabb) %>%  select(type, year, monthabb, AveragePrice) %>% 
...
seasonality.plot.conventional <- seasonal_df %>% select(season, year, AveragePrice, type) %>% 
  ...
  dplyr::summarize(avg=mean(AveragePrice)) %>% ggplot(aes(x=season, y=avg, color=season)) + geom_point(size=3) + 
...
regions_monthabb <- seasonal_df %>% select(monthabb, Total.Volume) %>% 
  dplyr::group_by(monthabb) %>% dplyr::summarize(avg.volume=mean(Total.Volume)) %>%
...
price_monthabb <- seasonal_df %>% select(monthabb, AveragePrice) %>% 
  dplyr::group_by(monthabb) %>% dplyr::group_by(avg.price=mean(AveragePrice)) %>%
...
volprice_diff <- seasonal_df %>% dplyr::group_by(year, monthabb) %>% select(year, monthabb, type, Volume_Price_Diff) %>% 
  filter(year == c("2015", "2016", "2017")) %>%
  dplyr::summarize(avg.diff=mean(Volume_Price_Diff)) %>%
...
volume_conv <- seasonal_df %>% select(year, monthabb, Total.Volume, region, type) %>% 
  ...
  dplyr::group_by(year, monthabb, region) %>% dplyr::summarize(avg.vol=mean(Total.Volume)) %>%
...
weekly_df <- original_avocado_df
weekly_df$week <- format(as.Date(original_avocado_df$Date), "%w")
